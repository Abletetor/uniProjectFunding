...
if (isset($_POST['idea-submit']) && !empty($_GET['p_id']) )
{
    ...
    $interestPercentage = $_POST['interest_percentage'];
    ...
    {   if($registerd_s=='yes'){
        ...
        $sql = "update posts set post_title=?, stage=?, resgister_name=?, post_date=?, post_content=?,post_type=?,post_tag=?,pitch_file=?, project_cost=?, interest_percentage=? where post_id=? ";
        $stmt = mysqli_stmt_init($conn);
        ...
        {
            mysqli_stmt_bind_param($stmt, "sssssssssss", $post_title,$s_level,$registerd_s,$post_date,$post_content,
            $post_type,$post_tag,$PitchFileNameNew,$projectCost,$interestPercentage,$_GET['p_id']);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_store_result($stmt);
